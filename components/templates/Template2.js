...
const Template2 = ({ data }) => {
  ...
  return (
    ...
    >
      ...
      <header className="bg-gradient-to-r from-blue-700 to-indigo-700 text-white p-8 rounded-lg mb-8 -mx-8 -mt-8">
      ...
      {professionalSummary && (
        <section className="mb-8">
          <div className="flex items-center mb-4">
            <div className="w-8 h-8 bg-blue-700 rounded-full flex-shrink-0 flex items-center justify-center mr-3">
              <User className="w-4 h-4 text-white" />
            </div>
            <h3 className="text-2xl font-bold text-gray-900">About Me</h3>
          </div>
          <p className="text-gray-700 leading-relaxed pl-11 border-l-4 border-blue-200 ml-4 whitespace-pre-wrap break-words overflow-wrap-anywhere">
            {professionalSummary}
          </p>
        </section>
      )}
        ...
        <section className="mb-8">
          ...
          <div className="pl-11 ml-4">
            {employmentHistory.map((job, i) => (
              <div key={i} className="relative mb-6 border-l-4 border-blue-200 pl-6">
                <div className="absolute w-3 h-3 bg-blue-700 rounded-full -left-2 top-2"></div>
      ...
      {education && education.length > 0 && education[0].school && (
        <section className="mb-8">
          ...
          <div className="pl-11 ml-4 space-y-4">
            {education.map((edu, i) => (
              <div key={i} className="bg-blue-50 p-4 rounded-lg border-l-4 border-blue-400">
                <h4 className="font-bold text-gray-900 text-lg break-words">{edu.degree}</h4>
                <p className="text-blue-700 font-medium break-words">{edu.fieldOfStudy}</p>
            ...
            <div className="pl-11 ml-4 space-y-2">
              {skills.map((s, i) => (
                <div key={i} className="flex items-center">
                  <Circle className="w-2 h-2 text-blue-500 flex-shrink-0 mr-3" fill="currentColor" />
            ...
            <div className="pl-11 ml-4 space-y-2">
              {languages.map((lang, i) => (
                <div key={i} className="flex justify-between items-center flex-wrap">
                  ...
                  <span className="text-sm text-blue-700 bg-blue-100 px-2 py-1 rounded whitespace-nowrap">
                    {lang.proficiency}
                  </span>
                </div>
              ))}
